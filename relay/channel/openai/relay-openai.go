...
func collectStreamFunctionCallNames(data string, seen map[string]struct{}, names *[]string) {
	...
	for _, choice := range streamResponse.Choices {
		for toolCallPosition, toolCall := range choice.Delta.ToolCalls {
			functionName := toolCall.Function.Name
			if functionName == "" {
				continue
			}
			toolCallIndex := toolCallPosition
			if toolCall.Index != nil {
				toolCallIndex = *toolCall.Index
			}
			toolCallKey := fmt.Sprintf("%d-%d", choice.Index, toolCallIndex)
			if _, alreadySeen := seen[toolCallKey]; alreadySeen {
				continue
			}
			seen[toolCallKey] = struct{}{}
