...
func TestQuotaClampAuditMapUsesJSONSafeSpecialValues(t *testing.T) {
	tests := []struct {
		name     string
		original float64
		want     string
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			clamp := &QuotaClamp{Op: "QuotaFromFloat", Kind: QuotaClampNaN, Original: testCase.original}
			auditMap := clamp.AuditMap()
			assert.Equal(t, testCase.want, auditMap["original"])
			_, err := Marshal(auditMap)
			require.NoError(t, err)
		})
	}
}
