...
func TestClaimQuotaForRefund_RejectsNonPositiveQuota(t *testing.T) {
	truncateTables(t)

	task := &Task{
		TaskID: "task_invalid_refund_claim",
		Status: TaskStatusFailure,
		Quota:  -100,
		Data:   json.RawMessage(`{}`),
	}
...
func TestMarkTasksFailedWithStatus_DoesNotOverwriteSuccess(t *testing.T) {
	truncateTables(t)

	pendingTask := &Task{TaskID: "task_pending_failure", Status: TaskStatusInProgress, Data: json.RawMessage(`{}`)}
	successfulTask := &Task{TaskID: "task_success_preserved", Status: TaskStatusSuccess, Progress: "100%", Data: json.RawMessage(`{}`)}
	insertTask(t, pendingTask)
	insertTask(t, successfulTask)

	rowsAffected, err := MarkTasksFailedWithStatus(
		[]int64{pendingTask.ID, successfulTask.ID},
...
func TestGetUnrefundedFailedTasks_FiltersAndLimits(t *testing.T) {
	...
	tasks := []*Task{
		...
		{TaskID: "failed_with_negative_quota", Status: TaskStatusFailure, Quota: -100, SubmitTime: TaskRefundLegacyCutoff, Data: json.RawMessage(`{}`)},
