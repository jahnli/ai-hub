...
### 规则 6: 变更日志 — 询问后记录实质性变更

当你在本项目中完成**实质性代码变更**后，**询问用户是否需要**在 `changes/` 中创建变更记录，由用户决定是否记录。不要自动追加。
...
- 编辑变更文档本身（`changes/` 目录下的文件变更不触发）
- `changes/` 目录出现在变更文件列表中时，应忽略该目录，仅评估其余文件

**完成代码变更后的步骤：**

1. 在 `changes/CHANGELOG.md` 表格末尾追加一行，编号递增：

| 编号 | YYYY-MM-DD | 一句话说明 | 二开变更 / 与上游一致 | 保留 / 检查 / 可丢弃 | 详情列 |
