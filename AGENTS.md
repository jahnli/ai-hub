...
### 规则 6: 变更日志 — AI 生成 + hook 兜底

变更日志采用两层机制保障：

1. **AI Agent 提交时**：必须在 `git add` 阶段同步生成语义化的 changelog 条目，描述变更的实际意图（如"LDAP 登录时飞书同步改为同步调用"），而非简单罗列文件路径。
2. **手动提交兜底**：`.githooks/pre-commit` 在检测到实质性代码变更但 `changes/` 未更新时，自动用路径推断生成基础条目并加入暂存区，提交不中断。

纯格式化/注释等无需记录的变更可用 `git commit --no-verify` 跳过。

**AI Agent 生成 changelog 的流程：**

在执行 `git commit` 之前，AI 必须：

1. 分析本次变更的语义（做了什么、为什么做），撰写一句话中文说明
2. 在 `changes/CHANGELOG.md` 表格末尾追加一行，编号为当前最大编号 +1
3. **详情列**根据变更规模选择格式：
   - **≤3 个文件**：直接在详情列写文件名，如 `` `common/constants.go`、`setting/theme.go` ``
   - **>3 个文件**：在 `changes/details/` 下创建 `{编号}-{短标识}.md`，详情列写 `[详情](details/xxx.md)` 链接
4. 将 changelog 文件 `git add` 进暂存区（hook 检测到 `changes/` 已更新会跳过自动生成）
...
**手动提交兜底规则（pre-commit hook）：**

当开发者手动提交且未更新 `changes/` 时，hook 自动生成基于路径推断的基础条目（如"前端更新（auth,channels）"）。事后可手动编辑使其更精确。
