...
ARG MODEL_SQUARE_AES_KEY
WORKDIR /build/web
COPY web/package.json web/bun.lock ./
RUN bun install --frozen-lockfile
COPY ./web ./
COPY ./VERSION /build/VERSION
RUN DISABLE_ESLINT_PLUGIN='true' VITE_REACT_APP_VERSION=$(cat /build/VERSION) MODEL_SQUARE_AES_KEY="${MODEL_SQUARE_AES_KEY}" bun run build
...
RUN go build -ldflags "-s -w -X 'github.com/QuantumNous/new-api/common.Version=$(cat VERSION)'" -o ai-gateway
...
COPY --from=builder2 /build/ai-gateway /
