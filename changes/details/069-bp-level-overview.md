...
- `web/src/features/users/components/users-mutate-drawer.tsx` — 目标角色为 AI BP/中心 BP 时展示「数据总览可见级别」数字输入框（0 不可见、正整数为第 N 级，不设上限），输入时按成员 `department_name` 层级深度实时钳制，附说明文案；顺带修复该文件既有 lint error（import type、promise catch、无用展开等）
- `web/src/i18n/locales/{en,zh,zh-TW,fr,ja,ru,vi}.json` — 补齐级别输入框标签与说明的 7 语言文案（移除带括号说明的级别选项键）
