...
- `controller/option.go` — 兼容并迁移旧版安全审计配置项。
- `controller/security_audit.go` — 提供审计配置和非工作时间请求查询接口。
- `model/option.go` — 增加旧配置向结构化配置迁移逻辑。
- `model/audit_setting_migration_test.go` — 覆盖安全审计配置迁移行为。
- `model/security_audit.go` — 聚合非工作时段请求，支持用户筛选、按天数排序及常用模型统计。
...
- `web/default/src/i18n/` — 补充安全审计相关多语言文案。
