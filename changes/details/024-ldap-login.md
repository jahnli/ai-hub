...
- `controller/ldap.go` — 新增 LDAP 登录控制器（认证、绑定、解绑）；LDAP 注册/绑定时同步用户公司字段，并按公司配置选择飞书或钉钉同步、邮箱后缀与自动订阅套餐；钉钉注册同步完成前不预填邮箱，绑定时异步刷新平台资料
...
- `setting/system_setting/ldap.go` — LDAP 连接相关系统设置（服务器、BaseDN、BindDN 等）；新增公司同步配置解析、归一化和按公司读取自动订阅套餐；支持保存钉钉 Client ID 与 Client Secret
...
- `web/default/src/features/system-settings/auth/oauth-section.tsx` — OAuth 设置区适配 LDAP 平台同步配置；公司配置表单按飞书/钉钉选择动态展示对应凭据，支持邮箱后缀和自动订阅套餐；为 LDAP 页签补充底部留白，避免添加配置按钮滚动到底部时被页面底部区域遮挡而无法点击
