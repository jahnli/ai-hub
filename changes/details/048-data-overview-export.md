...
- `web/default/src/features/data-overview/components/export-dialog.tsx` — 导出对话框组件重构：两个勾选项（子部门详情页、用户列表页）、数据获取与 Excel 生成
- `web/default/src/features/data-overview/lib/export-excel.ts` — Excel 生成核心逻辑：主表/子部门表/用户列表表构建、样式格式化、图表图片嵌入、文件命名与下载；导出费用改用系统汇率换算后的人民币金额
