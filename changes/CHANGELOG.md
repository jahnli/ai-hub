...
| 064 | 2026-07-20 | 新增 AI 中转站周报统计脚本：按使用日志历史计费快照拆分输入、输出与缓存 Token/费用，汇总均价、缓存命中率、费用 Top 5 模型及消费分布；消费分布计算抽取并复用于事业部统计脚本；支持上周/本周周期选择、内置模型归一化映射，并补充 Claude Sonnet 5 | `scripts/weekly_stats.py`、`scripts/secondary_department_stats.py` |
| 065 | 2026-07-20 | 新增安全审计：支持配置非工作时段、按用户和日期聚合异常请求、日志明细查看、用户名称模糊筛选与旧配置迁移；日志明细弹框高度统一为视口 85% | [详情](details/065-security-audit.md) |
