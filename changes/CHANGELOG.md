...
| 069 | 2026-08-17 | 数据总览可见部门级别改为按成员 bp_level 字段配置：移除中心 BP/事业部 AI BP 按角色固定级别的裁剪逻辑；保存时按 department_name 层级深度钳制；BP 未配置时隐藏入口并拒绝访问；部门树与报表范围统一按 bp_level 计算；用户编辑改用带部门名称的级别下拉框，并修复角色切换后抽屉内容横向溢出 | [详情](details/069-bp-level-overview.md) |
