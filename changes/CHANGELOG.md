...
| 055 | 2026-09-05 | 模型广场与分组定价优化：动态计费标签改为主题色；管理员可查看全部分组、分组倍率及 API 速率限制，普通用户仅能查看所属分组；模型卡片显示当前用户分组；模型详情基础价格按筛选分组或用户实际所属分组显示；新增分组 × 供应商倍率配置，以用户特殊倍率 > 供应商倍率 > 基础倍率的优先级统一计费、日志审计与模型广场价格展示，并提供可视化编辑器及多语言文案；模型广场完整定价响应改用环境变量密钥进行 AES-256-GCM 加密，前端解密并校验后展示，同时兼容加密响应的倍率同步与本地、Docker 部署；新增独立的超级管理员「模型广场配置」入口，支持按场景配置推荐模型、理由、排序和启停，广场按用户可见模型及筛选条件展示推荐，复用现有价格与详情交互 | [详情](details/055-pricing-square.md) |
