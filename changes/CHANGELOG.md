...
| 043 | 2026-08-15 | 数据总览增强：完善部门统计、使用分析、排行、日志、导出和权限裁剪；新增飞书、钉钉、无平台多公司配置与数据隔离；钉钉接口限 30 QPS，90018 时有限退避并脱敏错误；目录限并发，成员统计批量聚合，部门树按需懒加载；统一公司模式并优化首屏并行加载、缓存和成员分桶；总 Token 按四类字段合计；平台成员按主部门去重；BP/部门负责人按公司范围查看统计；修复租户查询、Token 过期、禁用账号统计及无订阅额度展示；新增固定筛选器、未搜索引导和模型系列关键字聚合，四张系列/模型图表置顶；全系统 Token 统计“均价”统一改为“单价”，按每亿 Token 计算并同步 API、排序、图表、导出及七语言文案 | [详情](details/043-data-overview.md) |
...
| 069 | 2026-08-13 | 数据总览可见部门级别改为按成员 bp_level 字段配置：移除中心 BP/事业部 AI BP 按角色固定级别（中心第 1 级、事业部第 2 级）的裁剪逻辑；users 表新增 bp_level（默认 0 不可见，正整数为第 N 级、不设上限）；保存时按 department_name 层级深度钳制，超过本人最深级自动收敛为最深级；BP 未配置（bp_level=0）时不显示入口：侧边栏隐藏菜单、路由跳转 403、后端中间件拒绝；部门树裁剪与报表推送范围统一按 bp_level 计算；用户创建/编辑支持配置该字段，BP 角色编辑时展示数字输入框 | [详情](details/069-bp-level-overview.md) |
| 070 | 2026-08-15 | 全系统 UI Tooltip 悬停弹出延迟统一为 100ms，消除不同模块即时或延迟过长的不一致体验 | [详情](details/070-tooltip-delay.md) |
