...
| 043 | 2026-08-19 | 数据总览增强：完善部门统计、使用分析、排行、日志、导出和权限裁剪；新增飞书、钉钉、无平台多公司配置与数据隔离；支持 BP 按显式部门配置跨公司查看范围；部门树按权限加载并允许跨公司节点展开；补齐数据总览七语言文案与用户编辑弹窗布局；数据总览日志表格隐藏渠道列，详情中的渠道、重试链和分组倍率仅超级管理员可见 | [详情](details/043-data-overview.md) |
