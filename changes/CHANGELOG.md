...
| 043 | 2026-07-24 | 数据总览页增强：完善部门统计、使用分析、用户排行、日志、导出和权限裁剪；新增公司配置管理以及飞书、钉钉、无平台多公司数据总览，按公司隔离组织树、成员缓存和统计权限；钉钉部门树与成员列表按应用和接口分别限制为 30 QPS，遇到 90018 限流时有限退避重试并脱敏错误；公司目录改为限并发加载，子部门统计改为并发收集成员并批量聚合用量，减少多公司及多部门查询耗时；部门树选择器改为懒加载：仅预拉取第一个公司的部门树，其余公司在选择器悬停时按需加载 | [详情](details/043-data-overview.md) |
