...
| 069 | 2026-08-19 | 数据总览可见部门级别改为按成员 bp_level 字段配置：移除中心 BP/事业部 AI BP 按角色固定级别的裁剪逻辑；保存时按 department_name 层级深度钳制；BP 未配置时隐藏入口并拒绝访问；部门树与报表范围统一按 bp_level 计算；用户编辑改用带部门名称的级别下拉框，并修复角色切换后抽屉内容横向溢出；修复同级重名部门（如多事业部下的第一开发部）导致 BP 树裁剪匹配到错误分支：改为按部门路径逐级匹配，路径缺失时依次去掉前导段重试并保留名称兜底 | [详情](details/069-bp-level-overview.md) |
