...
| 056 | 2026-07-04 | 使用日志表格与详情弹框优化：用户列增强，API 直接返回头像和显示名、头像悬停资料卡片、「耗时」列标题改为「耗时 / 首字」、列顺序优化；时间、渠道、令牌和详情列宽度微调；费用列订阅抵扣记录改为直接显示费用金额，悬停提示订阅来源；新增 IP 地址列并在移动端展示；详情列固定在右侧，错误日志详情文字显示错误色；渠道标签自动配色排除红色；日志详情弹框宽度调整为屏幕 50% | [详情](details/056-usage-logs-user-column.md) |
