...
describe('data overview entry access', () => {
  ...
  test('BP role passes only with configured visible departments', () => {
    ...
    assert.equal(
      canAccessDataOverview({
        role: ROLE.BU_BP,
        overview_dept_ids: ['dept:1:team', 'dept:1:bu'],
      }),
      true
    )
    assert.equal(
      canAccessDataOverview({ role: ROLE.BU_BP, overview_dept_ids: [] }),
      false
    )
  ...
  test('common users pass only as department leaders', () => {
    assert.equal(
      canAccessDataOverview({ role: ROLE.USER, is_dept_leader: true }),
      true
    )
  ...
  test('null user is denied', () => {
    assert.equal(
      canAccessDataOverview(null as unknown as DataOverviewAccessUser),
      false
    )
