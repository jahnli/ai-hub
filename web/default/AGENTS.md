...
- **类型检查**：git 提交前由 `.githooks/pre-commit` 自动执行 `bun run typecheck`，当暂存区包含 `web/default/` 下的 TS/TSX 文件时触发；若类型检查失败，提交将被阻止，须修复至无错误后重新提交。开发过程中无需每次编辑后手动执行 typecheck。新克隆仓库后需执行 `git config core.hooksPath .githooks` 激活。
...
- **2026-06-23**：类型检查改为 git 提交前由 `.githooks/pre-commit` 自动执行，不再要求每次编辑后手动运行。
